...
ohx_file = './demo_comb_df.pkl'
ohx_comb_df = pd.read_pickle(ohx_file)
...
oral_df = oral_comb_df[['id', 'dentition_status']]
oral_df
...
# ==========
# Merge demo and oral
merge_demo_oral = pd.merge(demo_comb_df, oral_df, on=['id'], 
                           how='left')
merge_demo_oral.rename(columns={'dentition_status': 'ohx_status'}, inplace=True)

# ============
# Report complete for exam_status == 2 and ohx_status == 1 
# or missing when ohx_status is missing or corresponds to partial/incomplete.
# missingness
merge_demo_oral['ohx'] = np.logical_and(
    merge_demo_oral['ohx_status'] == 'Complete', 
    merge_demo_oral['exam_status'] == 'Both interviewed and MEC examined'
...
# Replace the Bool variable according to dictionary
merge_demo_oral['ohx'] = merge_demo_oral['ohx'].replace({True: 'Complete', False: 'Missing'})
merge_demo_oral['ohx'] = pd.Categorical(merge_demo_oral['ohx'])


# ========
# derived variables
merge_demo_oral['under_20'] = merge_demo_oral['age'] < 20

college = ('College graduate or above', 'Some college or AA degree')
# if eduation variable in college: True (only those two situation)
# otherwise education variable is False
# replace: replace({np.nan: False})
merge_demo_oral['college'] = np.logical_or(
    merge_demo_oral['education'].apply(lambda x: x in college).replace({np.nan: False}),
    merge_demo_oral['under_20']
)
...
# +
# merge_demo_oral = merge_demo_oral.query('exam_status == "Both interviewed and MEC examined"')
# -
...
#
# - Include a column ‘p-value’ giving a p-value testing for a mean difference in age or an association beween each categorical varaible and missingness. 
# - Use a chi-squared test comparing the 2 x 2 tables for each categorical characteristic and OHX exam status and a t-test for the difference in age.
#
